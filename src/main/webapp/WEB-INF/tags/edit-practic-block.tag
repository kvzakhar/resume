...
<%@ attribute name="practic" required="false" type="net.simplesoft.resume.entity.Practic"%>
...
<div id="ui-item-${index}" class="panel panel-default">
	<div class="panel-body ui-item">
		...
		<div class="row">
			<resume:form-has-error path="items[${index}].position"/>
			...
			<resume:form-has-error path="items[${index }].company"/>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
			...
			<resume:form-has-error path="items[${index}].finishDate"/>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<div class="row">
					...
					<div class="col-xs-6">
						...
						<option value="">Not finished yet</option>
						<c:forEach var="year" items="${years}">
							<option value="${year}" ${year == practic.finishDateYear ? 'selected="selected"' : '' }>${yearM}</option>
						</c:forEach>
						</select>
					</div>
				</div>
